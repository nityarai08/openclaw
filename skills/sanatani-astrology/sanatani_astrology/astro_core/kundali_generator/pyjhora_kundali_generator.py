"""
PyJHora-based Kundali Generator Implementation.

This module provides a clean kundali generator implementation that uses the PyJHora
library as the primary calculation engine.
"""
...
_PYJHORA_ENV_FLAG = os.getenv("ENABLE_PYJHORA", "").strip().lower()
_PYJHORA_ENABLED = _PYJHORA_ENV_FLAG in {"1", "true", "yes"}
...
if _PYJHORA_ENABLED:
    try:
        ...
        drik = utils = charts = None  # type: ignore[assignment]
else:
    PYJHORA_AVAILABLE = False
    drik = utils = charts = None  # type: ignore[assignment]
    if _PYJHORA_ENV_FLAG:
        print("Warning: PyJHora disabled because ENABLE_PYJHORA flag is not truthy")
    else:
        print("Info: PyJHora support disabled by default; set ENABLE_PYJHORA=1 to enable.")
...
class PyJHoraKundaliGenerator(BaseKundaliGenerator):
    """
    Clean kundali generator implementation using PyJHora library.
    
    This implementation uses PyJHora as the primary calculation engine
    for all astrological calculations with consistent output schema.
    """

    GENERATOR_ID = 'pyjhora'
    GENERATOR_LABEL = 'PyJHora'
    
    def __init__(self):
        """Initialize the PyJHora-based kundali generator."""
        super().__init__()
        
        if not PYJHORA_AVAILABLE:
            raise ImportError(
                "PyJHora library is not available or not enabled. "
                "Install jhora and set ENABLE_PYJHORA=1 to use this implementation."
            )
    ...
    def _calculate_planetary_positions(
        ...
        """
        Calculate planetary positions using PyJHora.
        ...
        try:
            ...
            # Create PyJHora date object
            date_obj = drik.Date(birth_data.date.year, birth_data.date.month, birth_data.date.day)
            
            # Convert to Julian Day
            jd = utils.gregorian_to_jd(date_obj)
            
            # Add time component
            time_fraction = (birth_data.time.hour + birth_data.time.minute/60.0 + birth_data.time.second/3600.0) / 24.0
            jd_with_time = jd + time_fraction
            
            # Store Julian Day and place for downstream house calculations
            self._current_julian_day = jd_with_time
            self._current_place = place
            self._current_birth_details = birth_data
            
            # Calculate rasi chart (planetary positions)
            rasi_positions = charts.rasi_chart(
                jd_with_time,
                place,
                ayanamsa_mode=ayanamsa.upper(),
            )
            
            # PyJHora rasi_chart returns data in format: [planet_id, (rasi, longitude_in_rasi)]
            # where planet_id can be 'L' for Lagna or 0-8 for planets
            pyjhora_to_our_mapping = {
                'L': 'lagna',  # Lagna/Ascendant
                0: 'sun',      # Sun
                1: 'moon',     # Moon  
                2: 'mars',     # Mars
                3: 'mercury',  # Mercury
                ...
                5: 'venus',    # Venus
                6: 'saturn',   # Saturn
                7: 'rahu',     # Rahu (North Node)
                8: 'ketu'      # Ketu (South Node)
            }
            
            for pos_data in rasi_positions:
                if isinstance(pos_data, list) and len(pos_data) >= 2:
                    planet_id = pos_data[0]  # Planet identifier from PyJHora
                    if planet_id in pyjhora_to_our_mapping:
                        planet_name = pyjhora_to_our_mapping[planet_id]
                        if isinstance(pos_data[1], tuple) and len(pos_data[1]) >= 2:
                            rasi, longitude_in_rasi = pos_data[1]
                            # Convert to absolute longitude
                            absolute_longitude = rasi * 30 + longitude_in_rasi
                            positions[planet_name] = self._create_planetary_position(absolute_longitude)
            
        except Exception as e:
            print(f"Error calculating planetary positions with PyJHora: {e}")
            raise
    ...
    def _calculate_divisional_charts(
        ...
        """
        Calculate enhanced divisional charts using PyJHora-based positions.
        
        Args:
            planetary_positions: Basic planetary positions from PyJHora
        ...
        for chart_name, chart_info in divisional_factors.items():
            ...
            if chart_name == 'D1':
                ...
                # Add planetary relationships to each planet's data
    ...
    def _compute_d1_house_cusps(
        ...
        # BHAVA_CHALIT uses Sripati house system with midpoint-based planet placement
        if system == 'BHAVA_CHALIT':
            system = 'SRIPATI'
            use_midpoints = True
        # Prefer ephemeris-based Sripati cusps so PyJHora and ephemeris generators match.
        if system == 'SRIPATI' and getattr(self, '_current_julian_day', None) is not None:
            lat = getattr(birth_data, 'latitude', None)
            lon = getattr(birth_data, 'longitude', None)
            if lat is not None and lon is not None:
                try:
                    engine = getattr(self, '_ephemeris_engine', None)
                    if engine is None:
                        from .comprehensive_ephemeris_engine import ComprehensiveEphemerisEngine
                        engine = ComprehensiveEphemerisEngine()
                        self._ephemeris_engine = engine
                    cusps = engine.calculate_house_cusps(
        ...
        if not getattr(self, '_current_julian_day', None) or not getattr(self, '_current_place', None):
            return self._equal_house_cusps(lagna_longitude)
        ...
        method = method_map.get(system, None)
        if method is None:
            # Support explicit PyJHora method tokens for advanced users
            method = system
        ...
        try:
            bhava_chart = charts.bhava_chart(
                ...
                self._current_place,
                ayanamsa_mode=ayanamsa.upper(),
                bhava_madhya_method=method,
            )
            if bhava_chart:
                # PyJHora returns (start, cusp, end); use cusp for bhava midpoints when needed.
                index = 1 if use_midpoints else 0
                return [segment[1][index] % 360 for segment in bhava_chart]
        except Exception as exc:
            print(f"Warning: PyJHora house calculation failed with method {method}: {exc}")
    ...
    def _calculate_yogas_and_doshas(
        ...
        """
        Calculate basic yogas and doshas using PyJHora-based positions.
    ...
    def _calculate_dasha_periods(
        ...
        """
        Calculate Vimshottari dasha periods using PyJHora-based moon position.
        
        Args:
            birth_data: Birth details
            moon_position: Moon's position from PyJHora
        ...
        return {
            'birth_nakshatra': nakshatra_index + 1,
    ...
    def _get_implementation_info(self) -> Dict[str, Any]:
        """
        Get information about the PyJHora implementation.
        ...
        if PYJHORA_AVAILABLE:
            try:
                ...
                version = getattr(_package_info, '__version__', 'unknown')
            except:
                version = '4.5.0'  # Known version from pip list
        
        return {
            'method': 'PYJHORA',
            'library': 'PyJHora (jhora)',
            'version': version,
            'available': PYJHORA_AVAILABLE,
            'primary_engine': 'PyJHora',
            'fallback_methods': [],
            'swiss_ephemeris_available': False,
            'description': 'PyJHora-based astrological calculations using jhora module',
            'accuracy': 'high',
            'features': [
                'Precise planetary positions',
                'Accurate ascendant calculation',
                'Traditional Vedic methods',
                'Traditional ephemeris calculations',
    ...
    def _calculate_dignity(self, planet_name: str, rasi: int) -> str:
        ...
        dignity_map = {
            ...
            'mars': {9: 'Exalted', 5: 'Debilitated'},  # Capricorn exalted, VIRGO debilitated (not Cancer!)
            'mercury': {5: 'Exalted', 11: 'Debilitated'},  # Virgo exalted, Pisces debilitated
            'jupiter': {3: 'Exalted', 6: 'Debilitated'},  # Cancer exalted, LIBRA debilitated (not Capricorn!)
            'venus': {11: 'Exalted', 9: 'Debilitated'},  # Pisces exalted, CAPRICORN debilitated (not Virgo!)
    ...
    def _calculate_house_aspects(self, chart_positions: Dict[str, Any]) -> Dict[int, List[str]]:
        ...
        # All grahas aspect the 7th from themselves.
    ...
    def _calculate_planetary_relationships(self, chart_positions: Dict[str, Any]) -> Dict[str, Dict[str, str]]:
        ...
        """
        # Natural relationships: Friends, Enemies, Neutral
        ...
        for planet1 in all_planets:
            ...
            for planet2 in all_planets:
                ...
                # 1. Determine Temporary Relationship
                # Planets in houses 2, 3, 4, 10, 11, 12 from a planet are temporary friends.
                # relative_house is the distance from planet1's house to planet2's house.
                # 0 is same house, 1 is the next house (2nd from), etc.
                relative_house = (p2_house - p1_house + 12) % 12
                if relative_house in [1, 2, 3, 9, 10, 11]: # Corresponds to 2nd, 3rd, 4th, 10th, 11th, 12th houses from planet1
                    temp_status = 'friend'
                else:
                    temp_status = 'enemy'

                # 2. Get Natural Relationship
                ...
                # 3. Combine to get Five-Fold (Panchadha) Relationship
    ...
    # PyJHora-specific helper methods
    
    def _create_planetary_position(self, longitude: float) -> PlanetaryPosition:
        """
        Create PlanetaryPosition object from longitude.
        ...
        """
        # Convert longitude to rasi, nakshatra, etc.
        ...
        return PlanetaryPosition(
            ...
            retrograde=False  # PyJHora may provide this separately
        )
    
    def get_calculation_info(self) -> Dict[str, Any]:
        """Get information about calculation methods and availability."""
        return {
            'schema_version': self.schema_version,
            'implementation': self._get_implementation_info(),
            'supported_charts': list(range(1, 61)),  # D1 to D60
            'supported_yoga_types': [
                'Raja Yoga', 'Dhana Yoga', 'Mahapurusha Yoga', 'Gaja Kesari',
                'Chandra Mangal', 'Budh Aditya', 'Exchange Yoga', 'Neecha Bhanga'
            ],
            'supported_dosha_types': [
                'Manglik Dosha', 'Kaal Sarpa Dosha', 'Grahan Dosha',
                'Guru Chandal', 'Kemadrum Dosha', 'Papakartari Yoga'
            ],
